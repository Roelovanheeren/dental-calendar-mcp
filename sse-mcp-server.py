#!/usr/bin/env python3
"""
ElevenLabs SSE MCP Server

A Server-Sent Events (SSE) based MCP server that ElevenLabs can connect to.
"""
...
app = FastAPI(title="ElevenLabs SSE MCP Server", version="1.0.0")
...
async def root():
    ...
    return {
        "name": "Dental Calendar SSE MCP Server",
        "version": "1.0.0",
        "description": "Dental appointment management system with SSE support",
        "endpoints": {
            "health": "/health",
            "sse": "/sse",
            "mcp": "/mcp"
...
async def sse_endpoint():
    """Server-Sent Events endpoint for ElevenLabs."""
    async def event_generator():
        # Send initial connection message
        yield f"data: {json.dumps({'type': 'connected', 'message': 'Dental Calendar MCP Server connected'})}\n\n"
...
async def mcp_endpoint(request: Request):
    """Regular MCP endpoint for fallback."""
    try:
