#!/usr/bin/env python3
"""
ElevenLabs Compatible MCP Server

A server that implements the exact MCP protocol format that ElevenLabs expects.
"""
...
app = FastAPI(title="ElevenLabs Compatible MCP Server", version="1.0.0")
...
async def root():
    ...
    return {
        "name": "Dental Calendar MCP Server",
        "version": "1.0.0",
...
# Multiple endpoint formats for ElevenLabs compatibility
@app.post("/mcp")
@app.post("/tools")
@app.post("/tools/list")
@app.post("/tools/call")
async def mcp_endpoint(request: Request):
    ...
    try:
        ...
        elif method == "tools/list" or request.url.path.endswith("/tools/list"):
            return {
                "jsonrpc": "2.0",
                "id": body.get("id"),
                "result": {
        ...
        elif method == "tools/call" or request.url.path.endswith("/tools/call"):
